...
class PortalUsersDatabase : public Wt::Auth::AbstractUserDatabase
{
public:
  PortalUsersDatabase(dbo::Session& session);
...
private:
  ...
  struct WithUser {
    WithUser(const PortalUsersDatabase& self, const Wt::Auth::User& user);
    ~WithUser();
